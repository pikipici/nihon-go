...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    catch (e: any) { setError(e?.response?.data?.error ?? "Terjadi kesalahan. Coba lagi."); }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-8">
          ...
          <Divider label="atau masuk dengan email" className="my-5" />
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="mt-1.5 text-right">
                <Link href="/auth/forgot-password" className="text-xs text-purple-600 hover:underline">Lupa password?</Link>
              </div>
            </div>
            <Button type="submit" fullWidth loading={isSubmitting} className="mt-1">Masuk</Button>
          </form>
          <p className="mt-5 text-center text-sm text-gray-500">
            Belum punya akun?{" "}
            <Link href="/auth/register" className="font-medium text-purple-600 hover:underline">Daftar gratis</Link>
          </p>
        </div>
      </div>
    </div>
