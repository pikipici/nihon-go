...
const RATING = [
  ...
  { value: 2, label: "Sulit", color: "bg-amber-50 border-amber-200 text-amber-700", interval: "3 hr" },
  { value: 3, label: "Baik",  color: "bg-purple-50 border-purple-200 text-purple-700", interval: "8 hr" },
  { value: 4, label: "Mudah", color: "bg-green-50 border-green-200 text-green-700",  interval: "15 hr" },
];
...
export default function SrsPage() {
  ...
  useEffect(() => {
    api.get<{ cards: SrsCard[] }>("/srs/due-today")
  ...
  if (done) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <p className="text-sm text-gray-500 mb-4">{stats.correct}/{stats.total} benar · {Math.round(stats.correct/stats.total*100)}% akurasi</p>
      <Button onClick={() => { setIdx(0); setDone(false); setFlipped(false); api.get<{ cards: SrsCard[] }>("/srs/due-today").then(r => setCards(r.data.cards)); }}>
        Review lagi
      </Button>
    </div>
  );
  ...
  const card    = cards[idx]!;
  const display = card.vocabulary ?? card.kanji;
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="flex justify-between items-center mb-4 text-sm text-gray-500">
        <span>Kartu {idx + 1} / {cards.length}</span>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white overflow-hidden mb-4 cursor-pointer min-h-[220px] flex flex-col"
        onClick={() => setFlipped(f => !f)}>
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          <div className="text-xs font-medium text-purple-500 bg-purple-50 px-3 py-1 rounded-full mb-4">
            {card.itemType === "VOCABULARY" ? "語彙" : "漢字"} · {card.vocabulary?.jlptLevel ?? card.kanji?.jlptLevel}
          </div>
      ...
      {flipped ? (
        ...
        <Button fullWidth onClick={() => setFlipped(true)}>Tampilkan jawaban</Button>
      )}
    </div>
  );
}
